...
* Author: CRM Perks
* Text Domain: contact-form-salesforce
...
load_plugin_textdomain('contact-form-salesforce-crm', FALSE,  $this->plugin_dir_name(). '/languages/' );
...
self::$plugin->instance();
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo $id ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Salesforce Plugin','contact-form-salesforce-crm') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php _e('Dismiss Notice','contact-form-salesforce-crm') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo $id ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php _e('Contact Form Salesforce Plugin','contact-form-salesforce-crm') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sContact Form Entries%s is required. %s it today!%s", 'contact-form-salesforce-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Contact Form Entries is installed but not active. %sActivate Contact Form Entries%s to use the Contact Form Salesforce Plugin','contact-form-salesforce-crm'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=contact-form-entries-pro/contact-form-entries-pro.php'), 'activate-plugin_contact-form-entries-pro/contact-form-entries-pro.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sContact Form Entries%s is required. %sPurchase it today!%s", 'contact-form-salesforce-crm'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");
  }  
...
$value=trim(implode(' ',$value));
  ...
  */
  ...
  $link="N/A"; $desc=__("Added to ",'contact-form-salesforce-crm');
  ...
  $desc.=$feed['object'];
  if($feed['status'] == 5){
   $desc=sprintf(__("%s Deleted",'contact-form-salesforce-crm'),$feed['object']);   
  }if($feed['status'] == 4){
      $desc=sprintf(__("%s Filtered",'contact-form-salesforce-crm'),$feed['object']); 
  }
  }else{
  $desc= !empty($feed['error']) ? $feed['error'] : __("Unknown Error",'contact-form-salesforce-crm');
  }
  $title=__("Failed",'contact-form-salesforce-crm');   
  if($feed['status'] == 1){
  $title=__("Created",'contact-form-salesforce-crm');   
  }else if($feed['status'] == 2){
  $title=__("Updated",'contact-form-salesforce-crm');   
  }else if($feed['status'] == 4){
  $title=__("Filtered",'contact-form-salesforce-crm');   
  }else if($feed['status'] == 5){
  $title=__("Deleted",'contact-form-salesforce-crm');   
  }
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Salesforce Logs','contact-form-salesforce-crm').'" href="'.$log_url.'">'.__('Salesforce Logs','contact-form-salesforce-crm').'</a>';
...
return  $var;
  ...
  */
  ...
  if(class_exists('vxcf_sales_wp')){
      $status=1;
  }else if( file_exists(WP_PLUGIN_DIR.'/contact-form-salesforce-crm/contact-form-salesforce-crm.php')) {
  $status=2;
  } 
  return $status;
  }
...
$temp=apply_filters('crmperks_salesforce_post',$temp,$feed,$entry);
  ...
  if( $added){
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Salesforce (%s) with ID # %s .', 'contact-form-salesforce-crm'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Salesforce (%s) with ID # %s . View entry at %s', 'contact-form-salesforce-crm'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Salesforce', 'contact-form-salesforce-crm');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Salesforce (%s) with ID # %s . View entry at %s', 'contact-form-salesforce-crm'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Salesforce (%s) with ID # %s .', 'contact-form-salesforce-crm'),$feed['object'],$res['id']);   
            }
          if($event == 'delete'){  
     $screen_msg=__( 'Entry deleted from Salesforce', 'contact-form-salesforce-crm');
          }else{
     $screen_msg=__( 'Entry updated in Salesforce', 'contact-form-salesforce-crm');
          }
          }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'contact-form-salesforce-crm');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Salesforce. Entry not sent! Check the Entry Notes below for more details.' , 'contact-form-salesforce-crm' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Salesforce. Entry not sent' , 'contact-form-salesforce-crm' );    
  }
  $msg=sprintf(__('Error while creating %s', 'contact-form-salesforce-crm'),$feed['object']);
...
//  } 
  ...
  $log_link=' <a href="'.$log_url.'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','contact-form-salesforce-crm')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//$this->add_note($entry["id"], $msg);
  ...
  */
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Salesforce','contact-form-salesforce-crm')." Error","info_title"=>"More Detail","info"=>$detail);

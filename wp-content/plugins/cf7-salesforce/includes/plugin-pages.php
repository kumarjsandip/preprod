...
public function add_bulk_send_crm_callcenter($list){ 
 $list['send_salesforce']=__('Send to Salesforce','contact-form-salesforce-crm');
  return $list;  
}
...
$push=$this->push($entry_detail,$form,'add_note');
  ...
  ?>
  <div class="vx_row">
  <label><input type="checkbox" name="<?php echo $this->id ?>_send" value="yes"> <?php _e('Send to Salesforce','contact-form-salesforce-crm') ?></label>
  </div>
  <?php  
}
    /**
...
$lead['__vx_id']=$id;
   ...
   $slug=$this->get_slug();
      if ( $file == $slug ) {
          $settings_link=$this->link_to_settings();
            array_unshift( $links, '<a href="' .$settings_link. '">' . __('Settings', 'contact-form-salesforce-crm') . '</a>' );
        }
        return $links;
   }
  /**
  ...
  */
  ...
  // Adding submenu if user has access
        $page_title =__('Salesforce for Contact Form','contact-form-salesforce-crm');
        $menu_title =__('Salesforce','contact-form-salesforce-crm');
...
$class='';
 if($res){
       $msg=__('Account Deleted Successfully','contact-form-salesforce-crm');
  $class='updated';   
 }else{
       $msg=__('Error While Removing Account','contact-form-salesforce-crm');
  $class='error';      
 }
...
die();
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to add token','contact-form-salesforce-crm');
...
$info_data=$api->handle_code();
 ...
 /* if(!empty($token)){
  ...
  $msg=__('You do not have permissions to export logs','contact-form-salesforce-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  ...
  $fields=array(); $field_titles=array("#",__('Status','contact-form-salesforce-crm'),__('Salesforce ID','contact-form-salesforce-crm') ,__('Entry ID','contact-form-salesforce-crm'),__('Description','contact-form-salesforce-crm'),__('Time','contact-form-salesforce-crm'));
  ...
  check_admin_referer('vx_nonce','vx_nonce');
  if(!current_user_can($this->id."_edit_settings")){ 
  $msg=__('You do not have permissions to clear logs','contact-form-salesforce-crm');
  $this->display_msg('admin',$msg);
  return;   
  }
  $data=$this->get_data_object();
  $clear=$data->clear_logs();
       ...
       $msg=__('Error While Clearing Salesforce Logs','contact-form-salesforce-crm');
      $level="error";
      if(!empty($clear)){
      $msg=__('Salesforce Logs Cleared Successfully','contact-form-salesforce-crm');   
      $level="updated";
      }
  ...
  //
  self::$tooltips = array(
 'vx_feed_name' =>  __('Enter feed name of your choice.', 'contact-form-salesforce-crm'),
  'vx_sel_object' => __('Select the Object to Create when a Form is Submitted.', 'contact-form-salesforce-crm'),
   'vx_sel_account' =>__('Select the Salesforce account you would like to export entries to.', 'contact-form-salesforce-crm'),
  'vx_sel_form' => __('Select the Contact Form you would like to integrate with Salesforce. Contacts generated by this form will be automatically added to your Salesforce account.', 'contact-form-salesforce-crm'),
  
  'vx_map_fields' => __('Associate your Salesforce fields to the appropriate Contact Form fields.', 'contact-form-salesforce-crm'),
  
  'vx_optin_condition' =>__('When the opt-in condition is enabled, form submissions will only be exported to Salesforce when the condition is met. When disabled all form submissions will be exported.', 'contact-form-salesforce-crm'),
  
  'vx_manual_export' => __('If you do not want all entries sent to Salesforce, but only specific, approved entries, check this box. To manually send an entry to Salesforce, go to Entries, choose the entry you would like to send to Salesforce, and then click the "Send to Salesforce" button.', 'contact-form-salesforce-crm'),
  
    'vx_entry_notes' => __('Enable this option if you want to synchronize Contact Form entry notes to Salesforce Object notes. For example , when you add a note to a Contact Form entry, it will be added to the Salesforce Object selected in the feed.', 'contact-form-salesforce-crm'),
    
      'vx_primary_key' => __('Which field should be used to update existing objects?', 'contact-form-salesforce-crm'),
      
  'vx_web' => sprintf(__('Web-to-Lead is available for all Salesforce Editions. If you are not sure if your Salesforce Edition supports the API, you should use Web-to-Lead. Editions that do not support the Salesforce API: %s 1: Personal Edition %s 2: Group Edition %s 3: Professional Edition %s Note: You can purchase API access for a Professional Edition', 'contact-form-salesforce-crm'),'<br/>','<br/>','<br/>','<br/>'),
  'vx_api' => sprintf(__('The API features are more powerful than Web-to-Lead. You can create different object types, as well as other advanced features. If you have any of the following Salesforce Editions, you can use the included API Add-on: %s 1: Enterprise Edition %s 2: Unlimited Edition %s 3: Developer Edition %s 4: Professional Edition - Requires API Upgrade', 'contact-form-salesforce-crm'),'<br/>','<br/>','<br/>','<br/>','<br/>'),
  
  'vx_custom_app'=>__('This option is for advanced users who want to override default Salesforce App.','contact-form-salesforce-crm'),
  
  'vx_disable_logs'=>__('When an order is sent to Salesforce we store that order information in the database and show it in the Salesforce Log. Check this box if you do not want to save the exported order information in the logs.','contact-form-salesforce-crm'),
  
  'vx_line_items'=>__('Create a Salesforce Order product for each Woocommrce Cart item.','contact-form-salesforce-crm'),
  
  'vx_price_books'=>__('Get PriceBooks list from salesforce.','contact-form-salesforce-crm'),
  'vx_camps'=>__('Get Campaigns and Status list from salesforce.','contact-form-salesforce-crm'),
  
  'vx_sel_price_book'=>__('Which Pricebook should be searched for product','contact-form-salesforce-crm'),
  'vx_sel_camp'=>__('Which Campaign should be assigned to this object.','contact-form-salesforce-crm'),
  'vx_sel_status'=>__('What should be Member Status.','contact-form-salesforce-crm'),
  
  'vx_pro_desc'=>__('A new product will be created in selected Pricebook. You can add a description for new products created by this plugin.','contact-form-salesforce-crm'),
  
   'vx_assign_account'=>__('Enable this option if you want to assign an account this object.','contact-form-salesforce-crm'),
   'vx_sel_account'=>__('Object created by this feed will be assigned to the selected Account.','contact-form-salesforce-crm'),
   
      'vx_assign_contract'=>__('Enable this option , if you want to assign a Contract to this object','contact-form-salesforce-crm'),
   'vx_sel_contract'=>__('Select Contract feed. Contract created by this feed will be assigned to this object','contact-form-salesforce-crm'),
   
   'vx_camp_check'=>__('If enabled, Lead/Contact will be added to selected Campaign','contact-form-salesforce-crm'),
   'vx_owner_check'=>__('Enable this option if you want to assign another object owner.','contact-form-salesforce-crm'),
   'vx_owners'=>__('Get Users list from Salesforce','contact-form-salesforce-crm'),
   'vx_order_notes'=>__('Enable this option if you want to synchronize WooCommerce Order notes to Salesforce Object notes. For example, when you add a note to a WooCommerce Order, it will be added to the Salesforce Object selected in the feed.','contact-form-salesforce-crm'),
   'vx_sel_owner'=>__('Select a user as a owner of this object','contact-form-salesforce-crm'),
   
         'vx_entry_note'=>__('Check this option if you want to send more data as CRM entry note.', 'contact-form-salesforce-crm'),
   'vx_note_fields'=>__('Select fields which you want to send as a note', 'contact-form-salesforce-crm'),
   'vx_disable_note'=>__('Enable this option if you want to add note only for new CRM entry', 'contact-form-salesforce-crm'),
...
<div class="table_head_i"><?php _e('Capsule', 'contact-form-salesforce-crm'); ?></div>  
<div class="vx_group">
...
echo $this->format_log_msg($log);
 ...
 $msg=__('You do not have permissions for this action','contact-form-salesforce-crm');
...
if( $this->has_feed($form_id)) { 
 $boxes['salesforce_crm']=array('title'=>'<i class="fa fa-plug"></i> '.__('Salesforce', 'contact-form-salesforce-crm'),'callback'=>array($this,'meta_box_html'));  
} return $boxes; 
}
...
public function has_feed($form_id) {
 ...
 if(!empty($info['meta']) ){
  ...
  switch($action){
    ...
    $res['status']="ok";
    $data['crm_sel_camp']=$this->gen_select($camps,$status_sel,__('Select Campaign','contact-form-salesforce-crm'));
    ...
    if(is_array($camps)){
    $res['status']="ok";
    $data['crm_sel_status']=$this->gen_select($status_list,$camp_id_sel,__('Select Status','contact-form-salesforce-crm'));
  ...
  case"refresh_users":
    ...
    if(is_array($users)){
    $res['status']="ok";
    $data['crm_sel_user']=$this->gen_select($users,$owner_sel,__('Select User','contact-form-salesforce-crm'));
    $meta['users']=$users;   
    }else{
     $res['error']=$users;   
    }
...
if(isset($res['error'])){
    $res['status']='error';
    if(empty($res['error'])){
    $res['error']=__('Unknown Error','contact-form-salesforce-crm');
    }
}
  die(json_encode($res));    
  }
  ...
  */
  public  function mapping_page(){
      ...
      wp_enqueue_style('vxc-tooltip');
         $tabs=array('feeds'=>__('Salesforce Feeds','contact-form-salesforce-crm') , 'logs'=>__('Salesforce Log','contact-form-salesforce-crm') , 'accounts'=>__('Salesforce Accounts','contact-form-salesforce-crm'), 'settings'=>__('Settings','contact-form-salesforce-crm'));
  ...
  $(this).text('<?php _e('Show Key','contact-form-salesforce-crm') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php _e('Hide Key','contact-form-salesforce-crm') ?>');  
...
start_tooltip();  
  ...
  */
  ...
  _e('You do not have permissions to access this page','contact-form-salesforce-crm');    
  return;
  }
  ...
  <p>
  <?php esc_html_e("Feed deleted.", 'contact-form-salesforce-crm') ?>
  </p>
  </div>
  <?php
  }
  ...
  <div class="updated fade" style="margin:10px 0;">
  <p>
  <?php esc_html_e("Feeds deleted.", 'contact-form-salesforce-crm') ?>
  </p>
  </div>
  <?php
  }
...
$count=$this->data->delete_log($log_ids);
  $this->screen_msg(sprintf(__('Successfully Deleted %d Item(s)','contact-form-salesforce-crm'),$count));  
    }
  ...
  if(!empty($entry['__vx_data'])){ 
    ...
    $push=array('class'=>'error','msg'=>__('Entry Not Found','contact-form-salesforce-crm'));  
  }
  ...
  case"time": $time_order=$order_icon; $time_class="";   break;    
    ...
    $bulk_actions=array(""=>__('Bulk Action','contact-form-salesforce-crm'),"delete"=>__('Delete','contact-form-salesforce-crm'),
  'send_to_crm_bulk'=>__('Send to Salesforce','contact-form-salesforce-crm'),'send_to_crm_bulk_force'=>__('Force Send to Salesforce - Ignore filters','contact-form-salesforce-crm'));
  $base_url=$this->get_base_url();
   $objects=get_option($this->id.'_meta',array()); 

   $objects=$this->post('objects',$objects);
      $statuses=array("1"=>__("Created",'contact-form-salesforce-crm'),"2"=>__("Updated",'contact-form-salesforce-crm'),"error"=>__("Failed",'contact-form-salesforce-crm'),"4"=>__("Filtered",'contact-form-salesforce-crm'),"5"=>__("Deleted",'contact-form-salesforce-crm')); 
...
//
  ...
  $sel_fields=array(""=>__("Standard Field",'contact-form-salesforce-crm'),"value"=>__("Custom Value",'contact-form-salesforce-crm'));
  ...
  */
  ...
  public function set_logging_supported($plugins) {
      $slug=$this->plugin_dir_name(); 
        $plugins[$slug] = __('Salesforce','contact-form-salesforce-crm');
        return $plugins;
    }
  /**
...
$this->ajax=true;
  ...
  $msg=__("Please Choose Object",'contact-form-salesforce-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-salesforce-crm');
  }
  if($msg !=""){
  ?>
...
/*  $object=$this->post('object',$feed);
  ...
  $msg=__("Please Choose Account",'contact-form-salesforce-crm');
  }else  if(empty($_REQUEST['form_id'])){
  $msg=__("Please Choose Form",'contact-form-salesforce-crm');
  }
  if($msg !=""){
...
$field_options="<option>".__("Select Object",'contact-form-salesforce-crm')."</option>"; 
  if(is_array($objects)){
...
die();
  ...
  */
  ...
  if(!current_user_can($this->id.'_read_settings')){
  $msg_text=__('You do not have permissions to access this page','contact-form-salesforce-crm');   
  ...
  $this->uninstall();
  $uninstall_msg=sprintf(__("Contact Form Salesforce Plugin has been successfully uninstalled. It can be re-activated from the %s plugins page %s.", 'contact-form-salesforce-crm'),"<a href='plugins.php'>","</a>");
$this->screen_msg($uninstall_msg);
  ...
  $meta=isset($_POST['meta']) ? $this->post('meta') : array();
   ...
   $msgs['submit']=array('class'=>'updated','msg'=>__('Settings Changed Successfully','contact-form-salesforce-crm'));
...
include_once(self::$path . "templates/settings.php");
 ...
 $offset=$this->time_offset();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id."_edit_settings")){ 
  _e('You do not have permissions to save settings','contact-form-salesforce-crm');
  return;   
  }
  ...
  if(!$valid_email){
      $msgs['submit']=array("class"=>"error","msg"=>__('Invalid Email(s)','contact-form-salesforce-crm'));
  }
   $info=$this->get_info($id); 
   $crm=$info['data'];
...
//  $force_check=true;
  ...
  if($con_class != "updated" ){
      $msg=__('Connection to Salesforce is NOT Working','contact-form-salesforce-crm');  
  }else{
     $msg=__('Connection to Salesforce is Working','contact-form-salesforce-crm');   
  }
  $title=__('Test Connection: ','contact-form-salesforce-crm');
...
$msgs=array();
  ...
  check_admin_referer("vx_nonce");
  if(!current_user_can($this->id.'_edit_feeds')){
  _e('You do not have permissions to edit/save feed','contact-form-salesforce-crm'); 
  return;
  }
  //
...
if(is_array($feed_update) && is_array($feed)){
  ...
  if($is_valid){
      ...
      $msgs['save']=array('class'=>'updated','msg'=>sprintf(__("Feed Updated. %sback to list%s", 'contact-form-salesforce-crm'), '<a href="'.$feed_link.'">', "</a>"));
  }
  else{
  $msgs['save']=array('class'=>'error','msg'=>__("Feed could not be updated. Please enter all required information below.", 'contact-form-salesforce-crm'));
...
if(is_array($tags)){
  ...
  $fields['__vx_id']=array('id'=>'__vx_id','label'=>__('Entry ID','contact-form-salesforce-crm')); 
$fields['_vx_form_id']=array('id'=>'_vx_form_id','label'=>__('Form ID','contact-form-salesforce-crm')); 
$fields['_vx_form_name']=array('id'=>'_vx_form_title','label'=>__('Form Title','contact-form-salesforce-crm')); 
$fields['_vx_url']=array('id'=>'_vx_url','label'=>__('Page URL','contact-form-salesforce-crm')); 
$fields['_vx_created']=array('id'=>'_vx_created','label'=>__('Entry Created','contact-form-salesforce-crm')); 
$fields['_vx_updated']=array('id'=>'_vx_updated','label'=>__('Entry Updated','contact-form-salesforce-crm')); 

  $this->fields=$fields=array('cf'=>array("title"=>__('Contact Form Fields','contact-form-salesforce-crm'),"fields"=>$fields));
  ...
  $fields['feeds']=array("title"=>__('ID from other Feeds','contact-form-salesforce-crm'),"fields"=>$feeds);
  } }
  $this->fields=$fields;
  }
  return $fields;
...
if( in_array($option_k,$sel_val)){
  ...
  */
  ...
  $modules=array(""=>__("Select Object",'contact-form-salesforce-crm'));
...
$api_check=(int)$this->post('valid_api',$info);
  ...
  //////////////////
  if($res === NULL){
  $info['msg']=__('Invalid Org ID. Please check your Org ID and try again.','contact-form-salesforce-crm'); 
  $info['class']="error";        
  }else{
  $info['msg']=__('Success: Your Org ID is properly configured','contact-form-salesforce-crm'); 
  $info['class']="updated";
  }  
  ...
  $msg=__( 'Successfully Connected to Salesforce','contact-form-salesforce-crm' );
